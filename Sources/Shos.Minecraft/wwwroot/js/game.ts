...
function main(): void {
    ...
    async function loadAndRenderChunk(scene: any, worldId: string, x: number, y: number, z: number): Promise<IChunk> {
        ...
        const meshData = await requestChunkMesh(fetched.chunk.blocks.slice());
    ...
    function initializeGame(): void {
        ...
        loadAndRenderChunk(scene, DEV_PLACEHOLDER_WORLD_ID, 0, 0, 0).then((chunk) => {
            ...
            const playerController: IPlayerControllerModule = (window as any).ShosMinecraft.PlayerController;
