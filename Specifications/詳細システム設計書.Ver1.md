...
| 領域 | 責務 |
|---|---|
| フロントエンド(HTML5, TypeScript, Babylon.js) | 3D描画、メッシュ生成、物理・衝突判定、入力処理、レイキャストによるブロック操作、体力・落下ダメージ判定、取得済みチャンクのローカルキャッシュ |
| バックエンド(C#, ASP.NET Core Web API) | ワールドメタデータ・プレイヤー状態・インベントリの永続化、シード値管理、チャンク差分(変更済みブロック)の保存、地形生成ロジック |
| データベース(EF Core + SQL Server LocalDB) | `Worlds` / `Chunks` / `InventoryItems` テーブルによる永続化 |

地形生成はサーバー側が責務を持ち、クライアントは取得済みチャンクの描画とローカルキャッシュの更新に専念する。プレイヤー状態(位置・視点・体力・インベントリ・選択中ホットバー・初期スポーン地点)とチャンクの変更差分は、同一の保存処理でまとめて永続化し、状態の不整合が生じないようにする。
...
flowchart LR
    subgraph Client["クライアント(ブラウザ)"]
        Input[入力処理<br/>キーボード/マウス]
        Physics[物理・衝突判定<br/>AABB / 落下ダメージ]
        Raycast[レイキャスト<br/>ブロック破壊/設置]
        Render[Babylon.js 描画<br/>メッシュ生成/カリング]
        Cache[ローカルチャンクキャッシュ]
        Worker[Web Worker<br/>メッシュ生成]
    end

    subgraph Server["サーバー(ASP.NET Core Web API)"]
        WorldApi[Worlds API]
        ChunkApi[Chunks API]
        StateApi[State API]
        Terrain[地形生成<br/>パーリンノイズ]
    end
    ...
    Input --> Physics --> Raycast --> Cache
    Cache --> Worker --> Render
    Cache <-->|チャンク取得/保存| ChunkApi
    Physics -->|30秒ごと / 画面遷移前| StateApi
    WorldApi --> WorldsTable
    ChunkApi --> ChunksTable
    ChunkApi --> Terrain
    StateApi --> WorldsTable
    StateApi --> InventoryTable
```
...
- 地形生成はサーバー側が責務を持ち、クライアントは取得済みチャンクの描画とローカルキャッシュ更新に専念する。
...
- **Step 3**: レイキャストによるブロック破壊・設置(道具条件・破壊時間を含む)
