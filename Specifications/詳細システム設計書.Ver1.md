...
| Views | Razor の `cshtml` で HTML/CSS/TypeScript(コンパイル後のJS)の読み込みと画面構成を定義する |
...
- フロントエンドの実装言語は **TypeScript** とし、`.ts` ファイルで実装する。素の `.js` を直接手書きしない。
- コンパイルには `Microsoft.TypeScript.MSBuild`(NuGet パッケージ)を使用し、`dotnet build` の実行時に `.ts` を自動的に `.js` へトランスパイルする。Node.js / npm を用いる別建てのビルドツールチェーンは導入しない。
- `.ts` ソースは `wwwroot/js/` 配下にコンパイル後の `.js` と同居させ(`outDir` は指定しない)、`Views` からはコンパイル後の `.js` のみを読み込む。
- 型定義は `tsconfig.json` で管理し、DOM APIの標準型定義(`lib: ["ES2020", "DOM"]`)を用いる。
- **Babylon.js の型定義方針(ステップ4で確定)**: npm/Node.jsのビルドツールチェーンを導入しない方針(本節冒頭参照)のため、公式の `babylon.d.ts` はNuGet/npm経由で導入せず、ブラウザに `<script src="https://cdn.babylonjs.com/babylon.js">` で読み込む。TypeScript側は `BABYLON` をグローバルな `any` 型として扱い、個々のAPI呼び出しの型チェックは行わない。
...
- **不可視面のカリング(Face Culling)**: 各ブロックの6面のうち、隣接するブロックが不透明(空気・水以外)である面はメッシュ生成の対象から除外する。水(ID: 8)は非衝突かつ半透明のため、水に隣接する面は通常ブロックと同様にカリング判定の対象とする。チャンク境界に接する面(隣接チャンク未取得)は、隣接ブロックが不明なため外周面として描画対象に含める。
- **メッシュのデータ形式**: 頂点位置は `positions: Float32Array`(1頂点につき x, y, z の3要素)、法線は `normals: Float32Array`(1頂点につき3要素)、UVは `uvs: Float32Array`(1頂点につき u, v の2要素)、面の三角形分割は `indices: Uint32Array` として保持する(1チャンクの最大面数がUint16の範囲を超えうるため)。1面は4頂点・2三角形(6インデックス)で構成し、頂点順序は表側から見て反時計回りとする。
- **テクスチャアトラス(UVマッピング)**: 本ステップ(ステップ3)ではアトラス画像・アトラスのグリッド行数/列数は未定のため、各面に単位UV座標((0,0), (1,0), (1,1), (0,1))を割り当てるロジックのみを実装する。ブロックIDごとのアトラス内サブ矩形オフセットの算出は、アトラス画像とグリッド仕様を定義する後続ステップで追加する。
- **Web Worker によるバックグラウンドメッシュ生成**: チャンクの頂点・インデックスデータの生成処理をメインスレッドから切り離し、Web Worker上で実行することでメインスレッドのフレームレートを維持する。Workerへの入力は `blocks: Uint8Array`(チャンクのブロックID配列)、出力は `positions` / `normals` / `uvs` / `indices` の4つの `Transferable` な `ArrayBuffer` としてメインスレッドへ転送する。
- **描画への統合**: 本ステップで生成するメッシュデータをBabylon.js等のレンダラーへ実際に投入し画面へ描画する処理は、後続ステップ(ステップ4以降、Babylon.jsをCDN経由で導入した上で)の対象とする。
...
- **地表高さ・海面高・地中レイヤーの割り当て(ステップ4実装時にユーザー承認済みの暫定値)**:
  - 海面Y座標は `64` とする。
  - 地表Y座標は `64 + noise(x, z) * 32`(`noise` は各オクターブを正規化した -1〜1 の値)とし、地表高さの振幅は `32` ブロックとする。
  - 地表ブロックは、地表Yが海面以上なら草(ID: 2)、海面未満(水中の地表)なら砂(ID: 9)とする。
  - 地表から下4ブロックは土(ID: 1)、それより下は石(ID: 3)とする。
  - 地表Yが海面Y未満の場合、地表Yの直上から海面Yまでの区間は水(ID: 8)で満たし、海面Yより上は空気(ID: 0)とする。
  - 資源となるブロック(鉱石)は現行のブロックID表(0〜10)に定義がないため、本ステップでは生成しない。
- **`World.Seed` の導出(ステップ4時点の暫定措置。ユーザー承認済み)**: `Worlds` テーブルへの永続化(ステップ8)が実装されるまでの間、`GET /api/worlds/{id}/chunks` はリクエストの `WorldId` から決定的なハッシュ(FNV-1a)により `Seed` を導出する。ステップ8で `POST /api/worlds` によるシード生成・永続化が実装され次第、この暫定導出をワールドごとに永続化された `Seed` の参照に置き換える。
