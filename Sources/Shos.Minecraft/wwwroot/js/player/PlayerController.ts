...
interface IPlayerControllerModule {
    createPlayerController(scene: any, canvas: HTMLCanvasElement, chunk: IChunk, spawnX: number, spawnZ: number): IPlayerController;
}

(function (root: typeof globalThis, factory: (playerPhysics: IPlayerPhysicsModule) => IPlayerControllerModule): void {
    "use strict";
    if (typeof module === "object" && module && module.exports) {
        module.exports = factory(require("./PlayerPhysics.js"));
    } else {
        (root as any).ShosMinecraft = (root as any).ShosMinecraft || {};
        (root as any).ShosMinecraft.PlayerController = factory((root as any).ShosMinecraft.PlayerPhysics);
    }
})(globalThis, function (PlayerPhysics: IPlayerPhysicsModule): IPlayerControllerModule {
    "use strict";

    function createPlayerController(scene: any, canvas: HTMLCanvasElement, chunk: IChunk, spawnX: number, spawnZ: number): IPlayerController {
        ...
        const camera = new babylon.UniversalCamera(
            "playerCamera",
            new babylon.Vector3(state.position.x, state.position.y + PlayerPhysics.EYE_HEIGHT, state.position.z),
            scene);
        camera.minZ = 0.05;
        ...
        function onKeyDown(event: KeyboardEvent): void {
            switch (event.code) {
                case "KeyW": input.moveForward = true; break;
                case "KeyS": input.moveBackward = true; break;
                case "KeyA": input.moveLeft = true; break;
                case "KeyD": input.moveRight = true; break;
        ...
        function onKeyUp(event: KeyboardEvent): void {
            switch (event.code) {
                case "KeyW": input.moveForward = false; break;
                case "KeyS": input.moveBackward = false; break;
                case "KeyA": input.moveLeft = false; break;
                case "KeyD": input.moveRight = false; break;
